...
nginx_config_master_servers=""
nginx_config_slave_servers=""
IFS=$'\n'
for backendslave in $(cat config | grep backendslave)
do
	IFS=$' '
	back_a=($backendslave)
	nginx_config_slave_servers+="    server ${back_a[1]}:${back_a[2]};\n"
done
server_config="${server_config_base/TO_DO:/"$nginx_config_slave_servers"}" 

IFS=$'\n'
for backendmaster in $(cat config | grep backendmaster)
do
	IFS=$' '
	back_a=($backendmaster)
	nginx_config_master_servers+="    server ${back_a[1]}:${back_a[2]};\n"
done
server_config="${server_config/TO_DO2:/"$nginx_config_master_servers"}" 
